...
class TestPlacingAndConfirmingOrder:
    ...
    @pytest.mark.parametrize("name, family, address, phone, date, text", ORDER_TEST_DATA)
    def test_fill_order_form_with_different_data(self, order_page_driver, name, family, address, phone, date, text):
        driver = order_page_driver
        order_page = OrderPage(driver)
